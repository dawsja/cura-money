...
const EditMainSchema = z.object({ name: z.string().min(1).max(120) });
const AddSubSchema = z.object({
  ...
  planned: z.number().finite().min(0).optional(),
});
...
categoryRoutes.post(
  ...
  safe(async (c) => {
    ...
    const cat = await addMainCategory(
      userId(c),
      parsed.data.name,
      parsed.data.type,
      parsed.data.icon ?? 'Folder',
    );
...
categoryRoutes.post(
  ...
  safe(async (c) => {
    const body = await c.req.json().catch(() => null);
    const parsed = AddSubSchema.safeParse({ ...(body as object), mainCategoryId: routeParam(c, 'id') });
    if (!parsed.success) return badRequest(c, parsed.error.issues[0]?.message ?? 'invalid input');
    const sub = await addSubCategory(
      userId(c),
      parsed.data.mainCategoryId,
      parsed.data.name,
      parsed.data.planned ?? 0,
    );
...
categoryRoutes.delete(
  '/:id/sub/:subId',
  safe(async (c) => {
    await deleteSubCategory(userId(c), routeParam(c, 'subId'));
    return c.json({ ok: true });
  }),
);
