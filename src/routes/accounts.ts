...
accountRoutes.patch(
  ...
  safe(async (c) => {
    ...
    if (!parsed.success) return badRequest(c, parsed.error.issues[0]?.message ?? 'invalid input');
    // Only block an explicit move of minPayment to 0 on a credit/loan
    // account (same rule as /api/paydown). Changing `type` alone is
    // allowed even when minPayment is still 0 — SimpleFIN inserts and
    // create already land credit/loan rows that way, and the Paydown
    // calculator filters under-specified debts instead of crashing.
    if (parsed.data.minPayment !== undefined && parsed.data.minPayment <= 0) {
      ...
      if (existing) {
        ...
        if (nextType === 'credit' || nextType === 'loan') {
