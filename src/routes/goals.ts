...
import { z } from 'zod';
import { addGoal, deleteGoal, editGoal, getAccount, getAllGoals } from '@/db/queries';
import { userId, routeParam } from '@/lib/tenant';
import { badRequest, safe } from '@/lib/errors';
import { moneyAmount, positiveMoneyAmount } from '@/lib/money';

export const goalRoutes = new Hono();
const AddSchema = z.object({
  name: z.string().min(1).max(120),
  target: positiveMoneyAmount,
  startingValue: moneyAmount.default(0),
...
const EditSchema = z.object({
  ...
  target: positiveMoneyAmount.optional(),
  startingValue: moneyAmount.optional(),
...
goalRoutes.post(
  ...
  safe(async (c) => {
    ...
    if (!(await getAccount(userId(c), parsed.data.accountId)))
      return badRequest(c, 'accountId must belong to the current user');
