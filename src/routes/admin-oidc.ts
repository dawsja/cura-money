...
function callbackUriFor(c: { req: { raw: Request } }, providerId: string): string {
  const origin = env.APP_URL || env.BETTER_AUTH_URL
    ? env.APP_URL?.replace(/\/+$/, '') || env.BETTER_AUTH_URL?.replace(/\/+$/, '') || ''
...
adminOidcRoutes.post(
  ...
  safe(async (c) => {
    ...
    try {
      ...
      logger.error({ err: message }, 'admin-oidc: add failed');
...
adminOidcRoutes.patch(
  ...
  safe(async (c) => {
    ...
    // Re-validate the discovery doc if URL or secret changed (secret change
    // might be the IdP rotating, but the doc URL is what we talk to).
    if (parsed.data.discoveryUrl || parsed.data.clientSecret) {
      try {
        const res = await fetch(parsed.data.discoveryUrl ?? existing.discoveryUrl, {
          headers: { accept: 'application/json' },
        });
        if (!res.ok) {
          return badRequest(c, `OIDC discovery doc unreachable: ${res.status}`, 'discovery_unreachable');
        }
        ...
        return badRequest(c, `OIDC discovery doc unreachable: ${(err as Error).message}`, 'discovery_unreachable');
      }
    }
    ...
    if (parsed.data.clientSecret) patch.clientSecret = parsed.data.clientSecret;
    if (parsed.data.scopes) patch.scopes = parsed.data.scopes;
    if (parsed.data.isActive !== undefined) patch.isActive = parsed.data.isActive;
    await db.update(oidcProviders).set(patch).where(eq(oidcProviders.id, id));
    logger.info({ id, by: 'admin' }, 'admin-oidc: provider updated');
    // Hot-register: re-read providers from the DB and rebuild the auth
    // instance if the active set changed. Rename of providerId or change
    // to isActive both affect the active set; secret/URL/scope changes
    // don't (we don't include secret/URL/scope in the diff) so they'd be
    // a no-op rebuild — wasteful but safe.
...
adminOidcRoutes.delete(
  ...
  safe(async (c) => {
    ...
    // If we just removed the last active provider, the wizard step is no
    // longer satisfied. Flip the flag so the next time someone needs OIDC
    // they re-run the wizard step.
    const remaining = await db
      .select({ id: oidcProviders.id })
      .from(oidcProviders)
      .where(and(eq(oidcProviders.isActive, true)));
    if (remaining.length === 0) {
      await db.update(setupState).set({ oidcConfigured: false }).where(eq(setupState.id, 1));
    }
    ...
    const { changed } = await refreshAuth();
    return c.json({ ok: true, restart_required: !changed });
  }),
);
...
adminOidcRoutes.post(
  ...
  safe(async (c) => {
    ...
    if (!parsed.success) return badRequest(c, parsed.error.issues[0]?.message ?? 'invalid input');
    try {
      const res = await fetch(parsed.data.discoveryUrl.trim(), {
        headers: { accept: 'application/json' },
      });
      if (!res.ok) {
        return c.json(
          {
            ok: false,
            error: `Discovery doc unreachable: HTTP ${res.status}`,
