...
export function BudgetSummaryBox({
  ...
  const leftToBudget = plannedIncome - plannedExpense - plannedDebt;
  ...
  return (
    <div className="card overflow-hidden">
      <div className={clsx('px-5 py-6 text-center border-b border-inherit', headerBg)}>
        <div className={clsx('text-3xl font-bold tabular-nums', headerText)}>
          {overAssigned ? '−' : ''}{formatMoney(Math.abs(leftToBudget), true)}
        </div>
        <div className="mt-1 flex items-center justify-center gap-1 text-xs fg-muted">
          <span>Left to budget</span>
          <span title="Planned income minus planned expenses minus planned debt payments. Aim for $0.">
      ...
      <div className="p-5 space-y-5">
        ...
        <button
          ...
          className={clsx(
            'group w-full text-left rounded-lg p-2 -m-2 transition-colors',
