...
import { LogOut, Coffee, Settings } from 'lucide-react';
import clsx from 'clsx';
import { fetchMe, signOut, SIGNOUT_FLAG_KEY } from '../lib/auth';
import { ThemeToggle } from './ThemeToggle';

/**
 * Profile menu — replaces the old inline "Signed in as <email>" + sign
 * out button. Click the avatar to open a dropdown with:
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-2 rounded-full focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2 dark:focus:ring-offset-slate-900"
        aria-haspopup="menu"
        aria-expanded={open}
      ...
      {open && (
        ...
        >
          ...
          {/* Theme toggle — uses the labelled variant so the user knows
              what clicking it will do ("Dark mode" / "Light mode"). */}
          ...
          <div className="p-1.5">
            ...
            <a
              ...
              className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-amber-700 dark:text-amber-300 hover:bg-amber-50 dark:hover:bg-amber-900/30"
              role="menuitem"
            >
