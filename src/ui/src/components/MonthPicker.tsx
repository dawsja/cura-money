...
export function MonthPicker({ value, onChange, today, hideToday = false, disabled = false }: MonthPickerProps) {
  ...
  return (
    <div role="group" aria-label="Month selector" className="flex items-center gap-2">
      <Button
        variant="outline"
        size="icon"
      ...
      <div
        ...
        className="inline-flex h-9 min-w-[140px] items-center justify-center rounded-md border border-default bg-surface px-4 text-sm font-medium tabular-nums fg-primary"
      >
        {label}
      </div>
